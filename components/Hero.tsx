...
export const Hero: React.FC<HeroProps> = ({ lang, setView }) => {
  ...
  const radius = useMotionValue(40); // بداية بقطر صغير كتلميح
  ...
  useEffect(() => {
    if (isHovering) {
        ...
        if (isTouch) {
            ...
            // حالة النبض الهادئة للديسكتوب (كتلميح)
            radius.set(40);
            const pulse = animate(radius, [40, 60, 40], { duration: 4, repeat: Infinity, ease: "easeInOut" });
            return () => pulse.stop();
        }
    }
  }, [isHovering]);
  ...
  const handleCtaClick = (targetId: string) => {
      ...
      if (section) {
          ...
          setView('home'); // تأكيد أننا في الهوم أولاً
          setTimeout(() => {
